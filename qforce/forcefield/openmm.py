...
class OpenMM(ForcefieldSettings):
    ...
    def determine_bond_dissociation_energy(self, ids):
        ...
        elif self.ff.topo.edge(*ids)['type'] == '1(1.0)6':
            c_idx = np.where(elements == 6)[0][0]
            neighs = self.ff.elements[self.ff.topo.neighbors[0][c_idx]]
            n_hydrogen = len(neighs == 1)
            n_carbon = len(neighs == 6)
            n_oxygen = len(neighs == 8)

            if len(neighs) == 4 and n_hydrogen == 4:  # Methane
                e_dis = 431.
            elif n_carbon > 0:
                e_dis = 410.
            elif n_oxygen > 0:
        ...
        elif self.ff.topo.edge(*ids)['type'] == '6(2.0)8':
            e_dis = 732  # O=CH2

        elif self.ff.topo.edge(*ids)['type'] == '6(1.5)8':
            e_dis = 550  # amide guess
        elif self.ff.topo.edge(*ids)['type'] == '6(1.0)7':
            e_dis = 402  # amide guess
        elif self.ff.topo.edge(*ids)['type'] == '7(1.5)8':
            e_dis = 400  # NO3-

        elif self.ff.topo.edge(*ids)['type'] == '7(1.0)8':
            e_dis = 305  # NO2
        elif self.ff.topo.edge(*ids)['type'] == '8(1.5)15':
            e_dis = 450  # PLACEHOLDER for NO4 (3-)
        elif self.ff.topo.edge(*ids)['type'] == '1(1.0)15':
            e_dis = 351
        elif self.ff.topo.edge(*ids)['type'] == '6(1.0)6':
            e_dis = 368  # C-C
        elif self.ff.topo.edge(*ids)['type'] == '6(2.0)6':
            e_dis = 682  # C=C
        elif self.ff.topo.edge(*ids)['type'] == '8(2.0)16':
            e_dis = 551.4  # SO2
        elif self.ff.topo.edge(*ids)['type'] == '6(3.0)7':
            e_dis = 937
        elif self.ff.topo.edge(*ids)['type'] == '6(1.0)9':
            e_dis = 498
        else:
    ...
    def write_cross_bond_bond(self, forces, n_term):
        ...
        bb_cross_eq = 'k * (1-exp(-beta1*(distance(p1,p2)-r1_0))) * (1-exp(-beta2*(distance(p3,p4)-r2_0)))'
        ...
        ET.SubElement(per_bond_params, 'Parameter', {'name': 'k'})
        ET.SubElement(per_bond_params, 'Parameter', {'name': 'beta1'})
        ET.SubElement(per_bond_params, 'Parameter', {'name': 'beta2'})
        ...
        for cross_bb in self.ff.terms['cross_bond_bond']:
            ...
            beta1 = self.ff.beta
            beta2 = self.ff.beta
            k = - cross_bb.fconst * 100 / (beta1*beta2)
            k = str(round(k, 6))

            ET.SubElement(bb, 'Bond', {'p1': str(ids[0]), 'p2': str(ids[1]), 'p3': str(ids[2]), 'p4': str(ids[3]),
                                       'param1': equ1, 'param2': equ2, 'param3': k, 'param4': str(beta1), 'param5': str(beta2)})
    ...
    def write_cross_bond_angle(self, forces, n_term):
        if self.ff.cos_angle:
            ...
            ba_cross_eq = 'k * (cos(angle(p1,p2,p3))-cos(theta0)) * (1-exp(-beta*(distance(p4,p5)-r0)))'
        else:
        ...
        ET.SubElement(per_bond_params, 'Parameter', {'name': 'r0'})
        ET.SubElement(per_bond_params, 'Parameter', {'name': 'k'})
        ET.SubElement(per_bond_params, 'Parameter', {'name': 'beta'})
        ...
        for cross_ba in self.ff.terms['cross_bond_angle']:
            ...
            beta = self.ff.beta_ba
            k = -cross_ba.fconst * 10 / beta

            k = str(round(k, 7))

            ET.SubElement(ba, 'Bond', {'p1': str(ids[0]), 'p2': str(ids[1]), 'p3': str(ids[2]), 'p4': str(ids[3]),
                                       'p5': str(ids[4]), 'param1': equ1, 'param2': equ2, 'param3': k, 'param4': str(beta)})
