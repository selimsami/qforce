...
def fit_dipoles(inp):
    ...
    fit = Fit(model, qm_x=dipoles[:, 0], qm_y=dipoles[:, 1], qm_z=dipoles[:, 2],
...
#    print(q_fitted)
    ...
    fitted_dipole = np.append(fitted_dipole, [np.sqrt(fitted_dipole[0]**2+fitted_dipole[1]**2 +
                                                      fitted_dipole[2]**2)], axis=0)
...
def write_gmx_atoms(qm, q_fitted, job_name, mol):
    ...
    mass = [round(ATOMMASS[i], 4) for i in qm.atomids]
    atoms = [ATOM_SYM[i] for i in qm.atomids]
