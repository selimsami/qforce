...
class WriteORCA(WriteABC):
    def hessian(self, file, job_name, config, coords, atnums):
        ...
        file.write("%maxcore  {}\n\n".format(int(config.memory / config.n_proc * 0.75)))
    ...
    def scan(self, file, job_name, config, coords, atnums, scanned_atoms, start_angle, charge, multiplicity):
...
class ReadORCA(ReadABC):
    ...
    def _read_orca_allxyz(coord_file):
        ...
        # Updated code to allow reading general xyz trajectories (structures not separated by > like in ORCA)
        with open(coord_file, "r") as f:
            coord_text = f.read()
        coords = []
        ...
        for line in coord_text.splitlines():
            if line.strip("\n").strip().isdigit() and block:
                counter += 1
                n_atoms, elements, coord = ReadORCA._read_orca_coords("\n".join(block))
                coords.append(coord)
                block = [line]
                continue
            if ">" in line:
    ...
    def _read_orca_bond_order(out_file, n_atoms):
        ...
        """
        # Matches patterns like B(  0-Cl,  1-C ) :   1.0623
    ...
    def scan(self, config, file_name):
        ...
        if config.charge_method == "esp":
            n_atoms, charges = self._read_orca_esp("{}_charge.pc_chelpg".format(base))
            point_charges["esp"] = charges
        n_atoms, elements, coords = self._read_orca_allxyz("{}_scan.allxyz".format(base))
